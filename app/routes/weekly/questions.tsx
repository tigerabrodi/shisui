...
export const meta: MetaFunction = () => {
  return {
    title: 'Questions',
    description: 'Questions for the weekly assessments.',
  }
}
...
export const action: ActionFunction = async ({
  ...
  await deleteQuestions(user.id, 'WEEKLY')
...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  const dbQuestions = await findQuestions(user.id, 'WEEKLY')
...
export default function Questions() {
  ...
  return (
    <>
      <h2 className="heading-two">Questions for your weekly assessments.</h2>
      <Form
        action="/weekly/questions"
        method="post"
